...
const APIKEY = process.env.REACT_APP_API_KEY; // Your API Key goes here, which we put in .env
	...
	render() {
		return (
			<form onSubmit={ (event) => this.handleSubmit(event) }>
				<label htmlFor="">Search for art!</label>
				<input id="search" name="search" type="search" value={ this.state.searchInput } onChange={ (event) => this.handleChange(event) }/>
			</form>
		);
	}
}
